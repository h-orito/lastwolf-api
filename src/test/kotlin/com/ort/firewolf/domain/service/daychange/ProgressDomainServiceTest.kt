...
@SpringBootTest
class ProgressDomainServiceTest : FirewolfTest() {

    @Autowired
    lateinit var progressDomainService: ProgressDomainService
    ...
    fun test_addDayIfNeeded_必要なし_コミットなし設定() {
        ...
        // ## Act ##
        val afterDayChange = progressDomainService.addDayIfNeeded(dayChange, commits)
    ...
    fun test_dayChange_2日目() {
        ...
        val dayChange = DummyDomainModelCreator.createDummyDayChange().copy(
            ...
            abilities = VillageAbilities(
                listOf(
                    VillageAbility(yesterday.id, wolf.id, dummyParticipant.id, AbilityType(CDef.AbilityType.襲撃))
                )
            )
        )
    ...
    fun test_dayChange_3日目_人狼を処刑_村人を襲撃() {
        ...
        val dayChange = DummyDomainModelCreator.createDummyDayChange().copy(
            ...
            abilities = VillageAbilities(
                listOf(
                    VillageAbility(yesterday.id, wolf1.id, villager.id, AbilityType(CDef.AbilityType.襲撃)),
                    VillageAbility(yesterday.id, hunter.id, wolf2.id, AbilityType(CDef.AbilityType.護衛)),
                    VillageAbility(yesterday.id, seer.id, wolf2.id, AbilityType(CDef.AbilityType.占い))
                )
            ),
            votes = VillageVotes(
                listOf(
        ...
        assertThat(afterDayChange.abilities.list.filter { it.villageDayId == latestDay.id }).satisfies { latestDayAbilityList ->
            ...
            assertThat(latestDayAbilityList.any { it.abilityType.code == CDef.AbilityType.占い.code() }).isTrue()
            assertThat(latestDayAbilityList.any { it.abilityType.code == CDef.AbilityType.護衛.code() }).isTrue()
        }
    ...
    fun test_dayChange_4日目_人狼を処刑() {
        ...
        val dayChange = DummyDomainModelCreator.createDummyDayChange().copy(
            ...
            abilities = VillageAbilities(
                listOf(
                    VillageAbility(yesterday.id, wolf2.id, seer.id, AbilityType(CDef.AbilityType.襲撃)),
                    VillageAbility(yesterday.id, hunter.id, seer.id, AbilityType(CDef.AbilityType.護衛)),
                    VillageAbility(yesterday.id, seer.id, wolf2.id, AbilityType(CDef.AbilityType.占い))
                )
            ),
            votes = VillageVotes(
        ...
        assertThat(afterDayChange.abilities.list.filter { it.villageDayId == latestDay.id }).satisfies { latestDayAbilityList ->
            ...
            assertThat(latestDayAbilityList.none { it.abilityType.code == CDef.AbilityType.占い.code() }).isTrue()
            assertThat(latestDayAbilityList.none { it.abilityType.code == CDef.AbilityType.護衛.code() }).isTrue()
        }
