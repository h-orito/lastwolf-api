...
@SpringBootTest
class AttackDomainServiceTest : LastwolfTest() {

    @Autowired
    lateinit var attackDomainService: AttackDomainService
    ...
    fun test_getSelectingTarget_未参加者() {
        ...
        // ## Act ##
        val target = attackDomainService.getSelectingTarget(village, participant, villageAbilities)
    ...
    fun test_getSetMessage_対象あり() {
        ...
        // ## Act ##
        val message = attackDomainService.createSetMessage(chara, targetChara)
    ...
    fun test_getDefaultAbility_進行中でない() {
        ...
        // ## Act ##
        val villageAbilityList = attackDomainService.getDefaultAbilityList(village, villageAbilities)
    ...
    fun test_getDefaultAbility_狼が全員死亡() {
        ...
        // ## Assert ##
        assertThat(villageAbilityList).`as`("人狼が全員死亡しているのでなし").isEmpty()
    }
    ...
    fun test_getDefaultAbility_1日目() {
        ...
        // ## Assert ##
        val ability = villageAbilityList.firstOrNull()
        assertThat(ability).isNotNull()
        assertThat(ability!!.myselfId).`as`("生存している狼").isEqualTo(aliveWolf.id)
        assertThat(ability.targetId).`as`("初日はダミー固定").isEqualTo(dummyParticipant.id)
        assertThat(ability.abilityType?.code).isEqualTo(CDef.AbilityType.襲撃.code())
    }
    ...
    fun test_getDefaultAbility_対象なし() {
        ...
        // ## Assert ##
        assertThat(villageAbilityList).`as`("対象がいないのでなし").isEmpty()
    }
    ...
    fun test_getDefaultAbility_対象あり() {
        ...
        assertThat(ability).isNotNull()
        assertThat(ability!!.myselfId).`as`("生存している狼").isEqualTo(aliveWolf.id)
        assertThat(ability.targetId).`as`("生存者").isEqualTo(aliveVillager.id)
        assertThat(ability.abilityType.code).isEqualTo(CDef.AbilityType.襲撃.code())
    }
