...
class VillageCoordinator(
     ...
     * @param faceType 表情種別
     */
    fun confirmToSay(villageId: Int, user: LastwolfUser, messageText: String, messageType: String, faceType: String) {
     ...
     */
    @Transactional(rollbackFor = [Exception::class, LastwolfBusinessException::class])
    fun setComingOut(villageId: Int, user: LastwolfUser, skills: Skills) {
        // カミングアウトできない状況ならエラー
        val village: Village = villageService.findVillage(villageId)
        val participant: VillageParticipant? = findParticipant(village, user)
