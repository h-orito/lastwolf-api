...
data class PlayerRecords(
    ...
    companion object {
        private fun createCampRecordList(
            player: Player,
            villages: Villages,
        ): List<CampRecord> =
            CDef.Camp.listAll().sortedBy { Integer.parseInt(it.order()) }.map { camp ->
                val campVillageList =
                    villages.list.filter { village ->
                        val myCampCode =
        ...
        private fun createSkillRecordList(
            player: Player,
            villages: Villages,
        ): List<SkillRecord> =
            CDef.Skill
