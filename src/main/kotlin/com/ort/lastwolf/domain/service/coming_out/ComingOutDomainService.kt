...
class ComingOutDomainService {
    ...
    fun createComingOutMessage(
        chara: Chara,
        skill: Skill?,
        villageDayId: Int,
    ): Message = Message.createPublicSystemMessage(getComingOutSetMessage(chara, skill), villageDayId, true)

    // ===================================================================================
    //                                                                        Assist Logic
    //                                                                        ============
    private fun selectableSkillList(
        village: Village,
        participant: VillageParticipant?,
    ): List<Skill> {
        if (!isAvailableComingOut(village, participant)) return listOf()
        return village.setting.organizations
            .mapToSkillCount(village.participants.count)
            .filter { it.value > 0 }
            .keys
    ...
    private fun isAvailableComingOut(
        village: Village,
        participant: VillageParticipant?,
    ): Boolean {
        // 村として可能か
    ...
    private fun getComingOutSetMessage(
        chara: Chara,
        skill: Skill?,
    ): String {
