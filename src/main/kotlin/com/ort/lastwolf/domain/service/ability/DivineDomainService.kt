...
class DivineDomainService : IAbilityDomainService {
    ...
    fun divineRandomNowolf(
        ...
        val target =
            getSelectableTargetList(village, seer, dayChange.abilities)
                .filterNot {
    ...
    private fun createDivineMessageString(
        targetChara: Chara,
        isWolf: Boolean,
    ): String = "${targetChara.name.name}を占った。${targetChara.name.name}は人狼${if (isWolf) "の" else "ではない"}ようだ。"
    ...
    private fun isDivineKill(
        ...
        // 対象が既に死亡していたら呪殺ではない
        if (!dayChange.village.participants
                .first(targetId)
                .isAlive()
        ) {
            return false
